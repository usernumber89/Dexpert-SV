  ...
  // ── Status config ────────────────────────────────────────────────
  const STATUS = {
    PENDING:   { label: "Pendiente",   bg: "bg-[#1D5A9E]",  text: "text-white",  border: "border-white",  icon: Clock },
    ACCEPTED:  { label: "Aceptado",  bg: "bg-[#38A3F1]",  text: "text-white",  border: "border-[#1D5A9E]",  icon: CheckCircle },
    REJECTED:  { label: "Rechazado",  bg: "bg-[#F0F7FF]",    text: "text-[#1D5A9E]/60",    border: "border-red-200",    icon: XCircle },
    COMPLETED: { label: "Completado", bg: "bg-[#F0F7FF]", text: "text-[#0D5FA6]", border: "border-[#BAD8F7]",  icon: Star },
  } as const;
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-[#E8F3FD] bg-gradient-to-r from-[#F8FBFF] to-white">
        <p className="text-xs font-semibold uppercase tracking-widest text-[#93B8D4]">Perfil del Aplicante</p>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="px-6 py-5 space-y-5 -mt-4">
          ...
          {/* Historial / Estadísticas (NUEVO) */}
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-[#F8FBFF] rounded-xl border border-[#E8F3FD] p-4 flex flex-col items-center justify-center text-center">
              <div className="w-8 h-8 rounded-full bg-[#E8F3FD] flex items-center justify-center mb-2">
                <Briefcase className="w-4 h-4 text-[#0D3A6E]" />
              </div>
              <span className="text-xl font-bold text-[#0D3A6E] leading-none mb-1">
                {s.total_applications ?? 0}
              </span>
              <span className="text-[10px] font-bold uppercase tracking-widest text-[#93B8D4]">Proyectos Aplicados</span>
            </div>
            
            <div className="bg-[#F0F7FF] rounded-xl border border-[#BAD8F7] p-4 flex flex-col items-center justify-center text-center">
              <div className="w-8 h-8 rounded-full bg-[#E8F3FD] flex items-center justify-center mb-2">
                <CheckCircle className="w-4 h-4 text-[#38A3F1]" />
              </div>
              <span className="text-xl font-bold text-[#38A3F1] leading-none mb-1">
                {s.accepted_applications ?? 0}
              </span>
              <span className="text-[10px] font-bold uppercase tracking-widest text-[#93B8D4]">Aceptaciones Totales</span>
            </div>
          </div>
          ...
          {s.bio && (
            <div>
              <p className="text-[10px] font-bold uppercase tracking-widest text-[#93B8D4] mb-2">Acerca de</p>
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              { icon: GraduationCap, value: s.graduation_year ? `Promoción  ${s.graduation_year}` : null },
          ...
          {s.skills && s.skills.length > 0 && (
            <div>
              <p className="text-[10px] font-bold uppercase tracking-widest text-[#93B8D4] mb-2">Habilidades</p>
          ...
          <div className="space-y-2">
            {[
              ...
              { label: "Portafolio", href: s.portfolio,                 color: "text-[#38A3F1]" },
              { label: "Currículum",    href: s.resume_url,                color: "text-[#5B8DB8]" },
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              ...
              <p className="text-xs text-[#93B8D4]">{filtered.length} aplicante{filtered.length !== 1 ? "s" : ""}</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            {pending > 0 && (
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold text-[#0D3A6E] truncate">{s?.full_name ?? "Desconocido"}</p>
          ...
          <div className="max-w-4xl mx-auto px-4 sm:px-6 py-4 sm:py-8 space-y-4 sm:space-y-6">
              ...
              <div className="bg-white rounded-2xl border border-[#E8F3FD] px-5 py-4">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-sm font-semibold text-[#0D3A6E]">Desglose de aplicaciones</p>
                  </div>
                  <p className="text-xs text-[#38A3F1] font-medium">
                    {Math.round((metrics.accepted / metrics.total) * 100)}% tasa de aceptación
                  </p>
                </div>
            ...
            {filteredProjects.length === 0 ? (
              <div className="bg-white rounded-2xl border border-[#E8F3FD] py-20 flex flex-col items-center gap-3">
                ...
                <p className="text-sm font-semibold text-[#0D3A6E]">No aplicantes encontrados</p>
                <p className="text-xs text-[#93B8D4]">Intenta ajustar tu búsqueda o filtros</p>
              </div>
            ) : (
