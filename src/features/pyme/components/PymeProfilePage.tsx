...
import {
  ...
  Briefcase, TrendingUp, CheckCircle, Clock, Calendar
} from "lucide-react";
...
export default function PymeProfilePage() {
  ...
  const [stats, setStats] = useState<ProjectStats>({ total: 0, active: 0, completed: 0, totalApplications: 0 });
    ...
    try {
      ...
      if (projects) {
        setStats({
          ...
          totalApplications: projects.reduce((acc, p) => acc + ((p.applications as any[])?.length || 0), 0),
        });
      }
    } catch (error) {
      console.error(error);
  ...
  const handleUpload = async (
    ...
    field: keyof PymeProfile, setUploading: (v: boolean) => void
  ) => {
    if (!profile) return;
    setUploading(true);
    try {
      ...
      const { data: { publicUrl } } = supabase.current.storage.from(bucket).getPublicUrl(path);
      await supabase.current.from("pymes").update({ [field]: publicUrl }).eq("id", profile.id);
  ...
  if (!profile) return (
    <div className="min-h-screen bg-gradient-to-br from-[#F0F7FF] via-white to-[#E8F3FD] flex items-center justify-center p-6">
      ...
      >
        <div className="bg-white rounded-3xl border border-[#BAD8F7] shadow-2xl p-8 text-center">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#F0F7FF] via-white to-[#E8F3FD]">
      {/* Banner Mejorado */}
      <div className="relative h-56 bg-gradient-to-r from-[#0D3A6E] via-[#1D5A9E] to-[#0D3A6E] overflow-hidden">
        {/* Patrón de fondo */}
        <div className="absolute inset-0 opacity-10">
          <div className="absolute top-0 right-0 w-96 h-96 bg-white rounded-full blur-3xl" />
          <div className="absolute bottom-0 left-0 w-64 h-64 bg-[#38A3F1] rounded-full blur-3xl" />
        </div>
        ...
        {isEditing && (
          <motion.button
            ...
            className="absolute bottom-4 right-4 bg-white/95 backdrop-blur-sm p-3 rounded-xl shadow-lg hover:shadow-xl hover:scale-105 transition-all duration-200"
          >
            {uploadingBanner
              ? <div className="w-5 h-5 border-2 border-[#38A3F1] border-t-transparent rounded-full animate-spin" />
              : <Camera className="w-5 h-5 text-[#0D3A6E]" />}
          </motion.button>
        )}
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1">
            <motion.div 
              ...
              className="bg-white rounded-3xl border border-[#BAD8F7] shadow-xl overflow-hidden"
            >
              {/* Logo Section */}
              <div className="relative pt-8 pb-4 px-6">
                ...
                <input ref={logoInputRef} type="file" accept="image/*" className="hidden"
                  onChange={e => { const f = e.target.files?.[0]; if (f) handleUpload(f, "avatars", `logos/${profile.id}-${Date.now()}`, "logo_url", setUploadingLogo); }} />
              </div>
              ...
              <div className="grid grid-cols-2 gap-2 p-4 bg-[#F8FBFE] border-y border-[#E8F3FD]">
                <div className="bg-white rounded-xl p-3 text-center shadow-sm">
                  ...
                  <p className="text-[10px] text-[#93B8D4] uppercase tracking-wider">Projects</p>
                </div>
                <div className="bg-white rounded-xl p-3 text-center shadow-sm">
                  <div className="w-8 h-8 bg-[#F0F7FF] rounded-lg flex items-center justify-center mx-auto mb-2">
                    <Users className="w-4 h-4 text-[#F59E0B]" />
                  </div>
                  <p className="text-xl font-bold text-[#0D3A6E]">{stats.totalApplications}</p>
                  <p className="text-[10px] text-[#93B8D4] uppercase tracking-wider">Applications</p>
                </div>
                <div className="bg-white rounded-xl p-3 text-center shadow-sm">
