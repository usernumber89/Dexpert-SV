...
export async function POST(req: Request) {
  try {
    ...
    if (!checkRateLimit(getRateLimitKey(user.id, "boost-checkout"), 10, 60_000)) {
    ...
    const payload = {
      ...
      monto: BOOST_AMOUNT,
      nombreProducto: "Dexpert Student Boost - 30 días",
      configuracion: {
        urlRedirect: `${process.env.NEXT_PUBLIC_APP_URL}/student/dashboard?boost_success=true`,
    ...
    if (!paymentUrl) {
      console.error("Wompi no devolvió URL de pago");
