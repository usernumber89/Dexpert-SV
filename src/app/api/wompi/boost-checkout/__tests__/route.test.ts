...
describe("Boost Checkout", () => {
  ...
  it("debería retornar 401 si no hay usuario", async () => {
    ...
    const res = await POST(new Request("https://dexpert.app/api/wompi/boost-checkout", { method: "POST" }));
  ...
  it("debería retornar URL de pago en caso exitoso", async () => {
    ...
    mockFetch.mockResolvedValue({
      ok: true,
      json: () => Promise.resolve({ urlEnlace: "https://wompi.sv/pay/boost" }),
    });
    const { POST } = await import("../route");
    const res = await POST(new Request("https://dexpert.app/api/wompi/boost-checkout", { method: "POST" }));
    expect(res.status).toBe(200);
    const json = await res.json();
    expect(json.url).toBe("https://wompi.sv/pay/boost");
  });

  it("debería retornar 500 si Wompi no devuelve URL", async () => {
    mockGetUser.mockResolvedValue({ data: { user: { id: "user-1" } } });
    mockFrom.mockImplementation(() => ({
      select: () => ({ eq: () => ({ single: () => Promise.resolve({ data: { id: "student-1" }, error: null }) }) }),
    }));
